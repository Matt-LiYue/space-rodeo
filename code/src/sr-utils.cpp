...
float utils::linedotdistance( sf::Vector2f a, sf::Vector2f b, sf::Vector2f p)
{
  sf::Vector2f n = b - a;
  sf::Vector2f pa = a - p;
...
float utils::norm_sqrd(sf::Vector2f v) {
  return v.x * v.x + v.y * v.y;
}

float utils::norm(sf::Vector2f v) {
...
bool utils::within(float val, float x1, float x2) {
  if (x1 <= val and val <= x2) return true;
  else return false;
}

bool utils::withinBox(sf::Vector2f val, float x1, float x2, float y1, float y2) {
  if (within(val.x,x1,x2) && within(val.y,y1,y2)) return true;
  return false;
}

bool utils::intersects(sf::Vector2f p1, sf::Vector2f p2, sf::CircleShape cs) {
...
float utils::getLenToIntersect(sf::Vector2f p1, sf::Vector2f p2, sf::CircleShape cs) {
	sf::Vector2f c = cs.getPosition();
...
float utils::getLenToPerp(sf::Vector2f p1, sf::Vector2f p2, sf::Vector2f p3) {
